<div class="eltdf-instagram-list-holder <?php echo esc_attr($holder_classes); ?>">
    <?php if ( is_array( $images_array ) && count( $images_array ) ) { ?>
	    <ul class="eltdf-instagram-feed eltdf-outer-space <?php echo esc_attr($carousel_classes); ?> clearfix" <?php echo nigiri_elated_get_inline_attrs( $data_attr ) ?>>
	    <?php
    ...
    <?php } else { ?>
        <div class="eltdf-instagram-not-connected">
            <?php esc_html_e( 'It seams that you haven\'t connected with your Instagram account', 'nigiri-instagram-feed' ); ?>
        </div>
    <?php } ?>
</div>