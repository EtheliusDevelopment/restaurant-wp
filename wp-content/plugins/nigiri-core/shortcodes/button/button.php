<?php
namespace NigiriCore\CPT\Shortcodes\Button;
...
class Button implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Button', 'nigiri-core' ),
					'base'                      => $this->base,
					...
					'params'                    => array_merge(
						array(
							...
							array(
								...
								'heading'    => esc_html__( 'Enable Hover Animation', 'nigiri-core' ),
							...
							array(
								...
								'value'      => array(
									...
									esc_html__( 'Huge', 'nigiri-core' )    => 'huge'
								),
						...
						nigiri_elated_icon_collections()->getVCParamsArray( array(), '', true ),
						array(
							array(
								'type'       => 'colorpicker',
								'param_name' => 'color',
								'heading'    => esc_html__( 'Color', 'nigiri-core' ),
								'group'      => esc_html__( 'Design Options', 'nigiri-core' )
							),
							array(
								'type'       => 'colorpicker',
								'param_name' => 'hover_color',
								'heading'    => esc_html__( 'Hover Color', 'nigiri-core' ),
								'group'      => esc_html__( 'Design Options', 'nigiri-core' )
							),
							array(
								'type'       => 'colorpicker',
							...
							array(
								...
								'heading'    => esc_html__( 'Font Size (px)', 'nigiri-core' ),
								'group'      => esc_html__( 'Design Options', 'nigiri-core' )
							),
							array(
								'type'        => 'dropdown',
							...
							array(
								'type'        => 'textfield',
								'param_name'  => 'padding',
								'heading'     => esc_html__( 'Button Padding', 'nigiri-core' ),
								'description' => esc_html__( 'Insert padding in format: top right bottom left (e.g. 10px 5px 10px 5px)', 'nigiri-core' ),
								'dependency'  => array( 'element' => 'type', 'value' => array( 'solid', 'outline' ) ),
								'group'       => esc_html__( 'Design Options', 'nigiri-core' )
							)
						)
	...
	public function render( $atts, $content = null ) {
		...
		$params['size'] = ! empty( $params['size'] ) ? $params['size'] : 'small';
		...
		return nigiri_core_get_shortcode_module_template_part( 'templates/' . $params['html_type'], 'button', '', $params );
	}
	...
	private function getButtonStyles( $params ) {
		...
		if ( ! empty( $params['font_size'] ) ) {
			$styles[] = 'font-size: ' . nigiri_elated_filter_px( $params['font_size'] ) . 'px';
		}
	...
	private function getButtonClasses( $params ) {
		...
		if ( ( $params['animation'] ) === 'yes' ) {
			$buttonClasses[] = 'eltdf-has-hover-animation';
		}
