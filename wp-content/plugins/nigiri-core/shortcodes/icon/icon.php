<?php
namespace NigiriCore\CPT\Shortcodes\Icon;
...
class Icon implements Lib\ShortcodeInterface {
    ...
    public function vcMap() {
	    if(function_exists('vc_map')) {
		    vc_map(
		    	array(
				    'name'                      => esc_html__( 'Icon', 'nigiri-core' ),
				    'base'                      => $this->base,
				    ...
				    'params'                    => array_merge(
					    ...
					    array(
						    array(
							    'type'       => 'dropdown',
							    'param_name' => 'size',
							    'heading'    => esc_html__( 'Size', 'nigiri-core' ),
							    'value'      => array(
								    ...
								    esc_html__( 'Huge', 'nigiri-core' )   => 'eltdf-icon-huge'
							    )
						    ),
						    array(
							    'type'       => 'textfield',
							    'param_name' => 'custom_size',
							    'heading'    => esc_html__( 'Custom Size (px)', 'nigiri-core' )
						    ),
						    array(
							    'type'       => 'dropdown',
							    'param_name' => 'type',
						    ...
						    array(
							    ...
							    'heading'     => esc_html__( 'Border Radius', 'nigiri-core' ),
							    'description' => esc_html__( 'Please insert border radius(Rounded corners) in px. For example: 4 ', 'nigiri-core' ),
						    ...
						    array(
							    ...
							    'heading'    => esc_html__( 'Border Color', 'nigiri-core' ),
							    'dependency' => array( 'element' => 'type', 'value' => array( 'eltdf-circle', 'eltdf-square' ) )
						    ),
						    array(
							    'type'       => 'textfield',
							    'param_name' => 'border_width',
							    'heading'    => esc_html__( 'Border Width', 'nigiri-core' ),
							    'dependency' => array( 'element' => 'type', 'value'   => array( 'eltdf-circle', 'eltdf-square' ) )
						    ),
						    array(
							    'type'       => 'colorpicker',
						    ...
						    array(
							    ...
							    'heading'    => esc_html__( 'Hover Icon Color', 'nigiri-core' )
						    ),
						    array(
							    'type'       => 'colorpicker',
							    'param_name' => 'hover_border_color',
							    'heading'    => esc_html__( 'Hover Border Color', 'nigiri-core' ),
							    'dependency' => array( 'element' => 'type', 'value' => array( 'eltdf-circle', 'eltdf-square' ) )
						    ),
						    array(
							    'type'       => 'colorpicker',
							    'param_name' => 'hover_background_color',
							    'heading'    => esc_html__( 'Hover Background Color', 'nigiri-core' ),
							    'dependency' => array( 'element' => 'type', 'value' => array( 'eltdf-circle', 'eltdf-square' ) )
						    ),
						    array(
							    'type'        => 'textfield',
							    'param_name'  => 'margin',
							    'heading'     => esc_html__( 'Margin', 'nigiri-core' ),
						    ...
						    array(
							    ...
							    'heading'     => esc_html__( 'Use Link as Anchor', 'nigiri-core' ),
							    'value'       => array( 'Use this icon as Anchor?' => 'yes' ),
							    'description' => esc_html__( 'Check this box to use icon as anchor link (eg. #contact)', 'nigiri-core' ),
	...
	public function render( $atts, $content = null ) {
		...
		$html = nigiri_core_get_shortcode_module_template_part( 'templates/icon', 'icon', '', $params );
	...
	private function generateIconStyles( $params ) {
		...
		if ( ( $params['type'] !== 'eltdf-normal' && ! empty( $params['shape_size'] ) ) || ( $params['type'] == 'eltdf-normal' ) ) {
			if ( ! empty( $params['custom_size'] ) ) {
				$iconStyles[] = 'font-size:' . nigiri_elated_filter_px( $params['custom_size'] ) . 'px';
			}
		}
	...
	private function generateIconHolderStyles( $params ) {
		...
		if ( $params['type'] !== 'eltdf-normal' ) {
			...
			if ( ! empty( $shapeSize ) ) {
				$iconHolderStyles[] = 'width: ' . nigiri_elated_filter_px( $shapeSize ) . 'px';
				$iconHolderStyles[] = 'height: ' . nigiri_elated_filter_px( $shapeSize ) . 'px';
				$iconHolderStyles[] = 'line-height: ' . nigiri_elated_filter_px( $shapeSize ) . 'px';
			}
			...
			if ( ! empty( $params['border_color'] ) && ( isset( $params['border_width'] ) && $params['border_width'] !== '' ) ) {
				...
				$iconHolderStyles[] = 'border-width: ' . nigiri_elated_filter_px( $params['border_width'] ) . 'px';
			...
			if ( $params['type'] == 'eltdf-square' ) {
				if ( isset( $params['border_radius'] ) && $params['border_radius'] !== '' ) {
					$iconHolderStyles[] = 'border-radius: ' . nigiri_elated_filter_px( $params['border_radius'] ) . 'px';
				}
			}
		}
