<?php
namespace NigiriCore\CPT\Shortcodes\IconListItem;
...
class IconListItem implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'     => esc_html__( 'Icon List Item', 'nigiri-core' ),
					'base'     => $this->base,
					...
					'params'   => array_merge(
						array(
							...
							array(
								...
								'heading'     => esc_html__( 'Icon List Item Bottom Margin (px)', 'nigiri-core' ),
								'description' => esc_html__( 'Set bottom margin for your Icon List Item element. Default value is 8', 'nigiri-core' )
							)
						),
						...
						array(
							array(
								...
								'heading'    => esc_html__( 'Icon Size (px)', 'nigiri-core' )
							),
							array(
								'type'       => 'colorpicker',
							...
							array(
								...
								'heading'    => esc_html__( 'Title Size (px)', 'nigiri-core' ),
							...
							array(
								...
								'heading'     => esc_html__( 'Title Left Padding (px)', 'nigiri-core' ),
								'description' => esc_html__( 'Set left padding for your text element to adjust space between icon and text. Default value is 13', 'nigiri-core' ),
								'dependency'  => Array( 'element' => 'title', 'not_empty' => true )
							),
							array(
								'type'        => 'textfield',
	...
	public function render( $atts, $content = null ) {
		...
		$args   = array_merge( $args, nigiri_elated_icon_collections()->getShortcodeParams() );
		...
		$html = nigiri_core_get_shortcode_module_template_part( 'templates/icon-list-item-template', 'icon-list-item', '', $params );
	...
	private function getHolderStyles( $params ) {
		...
		if ( $params['item_margin'] !== '' ) {
			$styles[] = 'margin-bottom: ' . nigiri_elated_filter_px( $params['item_margin'] ) . 'px';
		}
	...
	private function getIconStyles( $params ) {
		...
		if ( ! empty( $params['icon_size'] ) ) {
			$styles[] = 'font-size: ' . nigiri_elated_filter_px( $params['icon_size'] ) . 'px';
		}
	...
	private function getTitleStyles( $params ) {
		...
		if ( ! empty( $params['title_size'] ) ) {
			$styles[] = 'font-size: ' . nigiri_elated_filter_px( $params['title_size'] ) . 'px';
		}
		
		if ( $params['title_padding'] !== '' ) {
			$styles[] = 'padding-left: ' . nigiri_elated_filter_px( $params['title_padding'] ) . 'px';
		}
