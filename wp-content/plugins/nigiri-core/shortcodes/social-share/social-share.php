<?php
namespace NigiriCore\CPT\Shortcodes\SocialShare;
...
class SocialShare implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Social Share', 'nigiri-core' ),
					'base'                      => $this->getBase(),
					...
					'params'                    => array(
						array(
							...
							'value'      => array(
								esc_html__( 'List', 'nigiri-core' )     => 'list',
								esc_html__( 'Dropdown', 'nigiri-core' ) => 'dropdown',
								esc_html__( 'Text', 'nigiri-core' )     => 'text'
							)
						),
						array(
							'type'       => 'dropdown',
							'param_name' => 'icon_type',
							'heading'    => esc_html__( 'Icons Type', 'nigiri-core' ),
							'value'      => array(
								esc_html__( 'Font Awesome', 'nigiri-core' ) => 'font-awesome',
								esc_html__( 'Font Elegant', 'nigiri-core' ) => 'font-elegant'
							),
						...
						array(
							'type'       => 'textfield',
							'param_name' => 'title',
							'heading'    => esc_html__( 'Social Share Title', 'nigiri-core' ),
	...
	public function render( $atts, $content = null ) {
		...
		$params['enable_social_share'] = ( nigiri_elated_options()->getOptionValue( 'enable_social_share' ) == 'yes' ) ? true : false;
		
		//Is social share enabled for post type
		$post_type         = get_post_type();
		$params['enabled'] = ( nigiri_elated_options()->getOptionValue( 'enable_social_share_on_' . $post_type ) == 'yes' ) ? true : false;
		...
		if ( $params['enable_social_share'] ) {
			if ( $params['enabled'] ) {
				$html .= nigiri_core_get_shortcode_module_template_part( 'templates/' . $params['type'], 'social-share', '', $params );
			}
		}
	...
	private function getSocialNetworksParams( $params ) {
		...
		foreach ( $this->socialNetworks as $net ) {
			...
			if ( nigiri_elated_options()->getOptionValue( 'enable_' . $net . '_share' ) == 'yes' ) {
				...
				$params['custom_icon'] = ( nigiri_elated_options()->getOptionValue( $net . '_icon' ) ) ? nigiri_elated_options()->getOptionValue( $net . '_icon' ) : '';
				
				$html = nigiri_core_get_shortcode_module_template_part( 'templates/parts/network', 'social-share', '', $params );
			}
    ...
    private function getSocialNetworkShareLink($net, $image) {
        switch ($net) {
            ...
            case 'twitter':
                ...
                $twitter_via = (nigiri_elated_options()->getOptionValue('twitter_via') !== '') ? ' via ' . nigiri_elated_options()->getOptionValue('twitter_via') . ' ' : '';
                if (wp_is_mobile()) {
                    $link = 'window.open(\'https://twitter.com/intent/tweet?text=' . urlencode(nigiri_elated_the_excerpt_max_charlength($count_char) . $twitter_via) . get_permalink() . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');';
                } else {
                    $link = 'window.open(\'http://twitter.com/home?status=' . urlencode(nigiri_elated_the_excerpt_max_charlength($count_char) . $twitter_via) . get_permalink() . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');';
                }
                break;
            case 'google_plus':
            ...
            case 'tumblr':
                ...
                break;
            case 'pinterest':
                $link = 'popUp=window.open(\'http://pinterest.com/pin/create/button/?url=' . urlencode(get_permalink()) . '&amp;description=' . nigiri_elated_addslashes(get_the_title()) . '&amp;media=' . urlencode($image[0]) . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false;';
                break;
            case 'vk':
	...
	private function getSocialNetworkText( $net ) {
		switch ( $net ) {
			case 'facebook':
				$text = esc_html__( 'fb', 'nigiri-core');
				break;
			case 'twitter':
				$text = esc_html__( 'tw', 'nigiri-core');
				break;
			case 'google_plus':
				$text = esc_html__( 'g+', 'nigiri-core');
				break;
			case 'linkedin':
				$text = esc_html__( 'lnkd', 'nigiri-core');
				break;
			case 'tumblr':
				$text = esc_html__( 'tmb', 'nigiri-core');
				break;
			case 'pinterest':
				$text = esc_html__( 'pin', 'nigiri-core');
				break;
			case 'vk':
				$text = esc_html__( 'vk', 'nigiri-core');
				break;
			default:
				$text = '';
		}
