<?php
namespace NigiriCore\CPT\Shortcodes\Banner;
...
class Banner implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Banner', 'nigiri-core' ),
					'base'                      => $this->getBase(),
					...
					'params'                    => array(
						...
						array(
							...
							'heading'    => esc_html__( 'Image Overlay Color', 'nigiri-core' )
						),
						array(
							'type'        => 'dropdown',
							'param_name'  => 'hover_behavior',
							'heading'     => esc_html__( 'Hover Behavior', 'nigiri-core' ),
							'value'       => array(
								esc_html__( 'Visible on Hover', 'nigiri-core' )   => 'eltdf-visible-on-hover',
								esc_html__( 'Visible on Default', 'nigiri-core' ) => 'eltdf-visible-on-default',
								esc_html__( 'Disabled', 'nigiri-core' )           => 'eltdf-disabled'
							),
							'save_always' => true
						),
						array(
							...
							'heading'     => esc_html__( 'Info Position', 'nigiri-core' ),
							'value'       => array(
								esc_html__( 'Default', 'nigiri-core' )  => 'default',
								esc_html__( 'Centered', 'nigiri-core' ) => 'centered'
							),
							'save_always' => true
						),
						array(
							...
							'heading'     => esc_html__( 'Info Content Padding', 'nigiri-core' ),
							'description' => esc_html__( 'Please insert padding in format top right bottom left', 'nigiri-core' )
						),
						array(
							'type'       => 'textfield',
							'param_name' => 'subtitle',
						...
						array(
							...
							'heading'    => esc_html__( 'Link Text', 'nigiri-core' ),
							'dependency' => array( 'element' => 'link', 'not_empty' => true )
						),
						array(
							'type'       => 'colorpicker',
							'param_name' => 'link_color',
							'heading'    => esc_html__( 'Link Text Color', 'nigiri-core' ),
							'dependency' => array( 'element' => 'link', 'not_empty' => true )
						),
						array(
							'type'       => 'textfield',
							'param_name' => 'link_top_margin',
							'heading'    => esc_html__( 'Link Text Top Margin (px)', 'nigiri-core' ),
							'dependency' => array( 'element' => 'link', 'not_empty' => true )
						)
					)
				)
	...
	public function render( $atts, $content = null ) {
		...
		$html = nigiri_core_get_shortcode_module_template_part( 'templates/banner', 'banner', '', $params );
	...
	private function getLinkStyles( $params ) {
		...
		if ( ! empty( $params['link_top_margin'] ) ) {
			$styles[] = 'margin-top: ' . nigiri_elated_filter_px( $params['link_top_margin'] ) . 'px';
		}
