<?php
namespace NigiriCore\CPT\Shortcodes\BlogSlider;
...
class BlogSlider implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		vc_map(
			array(
				'name'                      => esc_html__( 'Blog Slider', 'nigiri' ),
				'base'                      => $this->base,
				...
				'params'                    => array(
					array(
						...
						'value'       => array(
							esc_html__( 'Slider', 'nigiri' )            => 'slider',
							esc_html__( 'Carousel', 'nigiri' )          => 'carousel',
							esc_html__( 'Carousel Centered', 'nigiri' ) => 'carousel-centered'
						),
						'save_always' => true
					),
					array(
						...
						'heading'    => esc_html__( 'Number of Posts', 'nigiri' )
					),
					array(
						'type'        => 'dropdown',
						'param_name'  => 'orderby',
					...
					array(
						'type'        => 'autocomplete',
						'param_name'  => 'category',
						'heading'     => esc_html__( 'Category', 'nigiri' ),
						'description' => esc_html__( 'Enter one category slug (leave empty for showing all categories)', 'nigiri' )
					),
					array(
						'type'       => 'dropdown',
						'param_name' => 'image_size',
						'heading'    => esc_html__( 'Image Size', 'nigiri' ),
						'value'      => array(
							...
							esc_html__( 'Thumbnail', 'nigiri' ) => 'thumbnail',
					...
					array(
						...
						'group'       => esc_html__( 'Post Info', 'nigiri' )
					),
					array(
						'type'        => 'dropdown',
					...
					array(
						...
						'heading'     => esc_html__( 'Enable Post Info Author', 'nigiri' ),
					...
					array(
						...
						'heading'     => esc_html__( 'Enable Post Info Date', 'nigiri' ),
						'value'       => array_flip( nigiri_elated_get_yes_no_select_array( false, true ) ),
						'save_always' => true,
						'group'       => esc_html__( 'Post Info', 'nigiri' )
					),
					array(
						...
						'heading'     => esc_html__( 'Enable Post Info Category', 'nigiri' ),
						'value'       => array_flip( nigiri_elated_get_yes_no_select_array( false, true ) ),
						'save_always' => true,
						'group'       => esc_html__( 'Post Info', 'nigiri' )
					),
					array(
						...
						'heading'     => esc_html__( 'Enable Post Info Comments', 'nigiri' ),
	...
	public function render( $atts, $content = null ) {
		...
		nigiri_elated_get_module_template_part( 'shortcodes/blog-slider/holder', 'blog', '', $params );
    ...
    public function blogListCategoryAutocompleteSuggester( $query ) {
        ...
        if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
            foreach ( $post_meta_infos as $value ) {
                ...
                $data['label'] = ( ( strlen( $value['category_title'] ) > 0 ) ? esc_html__( 'Category', 'nigiri' ) . ': ' . $value['category_title'] : '' );
                $results[]     = $data;
            }
        }
        ...
        if ( ! empty( $query ) ) {
            ...
            if ( is_object( $category ) ) {
                ...
                if ( ! empty( $category_title ) ) {
                    $category_title_display = esc_html__( 'Category', 'nigiri' ) . ': ' . $category_title;
                }
