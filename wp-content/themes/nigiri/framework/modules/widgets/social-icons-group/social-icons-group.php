...
class NigiriElatedClassClassIconsGroupWidget extends NigiriElatedClassWidget {
	public function __construct() {
		parent::__construct(
			'eltdf_social_icons_group_widget',
			esc_html__( 'Nigiri Social Icons Group Widget', 'nigiri' ),
			array( 'description' => esc_html__( 'Use this widget to add a group of up to 6 social icons to a widget area.', 'nigiri' ) )
		);
	...
	protected function setParams() {
		$this->params = array_merge(
			...
			nigiri_elated_icon_collections()->getSocialIconWidgetMultipleParamsArray(6),
			array(
				array(
					'type'    => 'dropdown',
					'name'    => 'layout',
					'title'   => esc_html__( 'Icons Layout', 'nigiri' ),
					'options' => array(
						''             => esc_html__( 'Default', 'nigiri' ),
						'square-icons' => esc_html__( 'Square', 'nigiri' ),
				...
				array(
					'type'        => 'dropdown',
					'name'        => 'skin',
					'title'       => esc_html__( 'Square Icons Skin', 'nigiri' ),
					'description' => esc_html__( 'This applies to the square layout', 'nigiri' ),
					'options'     => array(
						''           => esc_html__( 'Dark Skin', 'nigiri' ),
						'light-skin' => esc_html__( 'Light Skin', 'nigiri' ),
					)
				),
				array(
					'type'    => 'dropdown',
					'name'    => 'alignment',
					'title'   => esc_html__( 'Text Alignment', 'nigiri' ),
					'options' => array(
				...
				array(
					'type'  => 'textfield',
					'name'  => 'icon_size',
					'title' => esc_html__( 'Icons Size (px)', 'nigiri' )
				),
				array(
					'type'  => 'colorpicker',
					'name'  => 'color',
				...
				array(
					'type'  => 'colorpicker',
					'name'  => 'border_color',
					'title' => esc_html__( 'Border Color (Outline Icons Layout Only)', 'nigiri' )
				),
				array(
					'type'  => 'colorpicker',
					'name'  => 'hover_border_color',
					'title' => esc_html__( 'Hover Border Color (Outline Icons Layout Only)', 'nigiri' )
				),
				array(
					'type'        => 'textfield',
					'name'        => 'margin',
	...
	public function widget( $args, $instance ) {
		...
		$hover_border_color = ! empty( $instance['hover_border_color'] ) ? $instance['hover_border_color'] : '';
		$class = implode( ' ', $class );
		...
		for ( $n = 1; $n <= 6; $n++ ) {
			...
			<?php if ( ! empty( $icon_holder_html ) ) { ?>
				<a class="eltdf-social-icon-widget-holder eltdf-icon-has-hover" <?php echo nigiri_elated_get_inline_attr( $hover_color, 'data-hover-color' ); ?> <?php echo nigiri_elated_get_inline_attr( $hover_border_color, 'data-hover-border-color' ); ?> <?php nigiri_elated_inline_style( $icon_styles ) ?>
				   href="<?php echo esc_url( $link ); ?>" target="<?php echo esc_attr( $target ); ?>">
					<?php echo wp_kses_post( $icon_holder_html ); ?>
					<?php if ($text !== '') { ?>
						<span class="eltdf-social-icon-widget-text"><?php echo esc_html($text); ?></span>
					<?php } ?>
				</a>
			<?php }
		}
