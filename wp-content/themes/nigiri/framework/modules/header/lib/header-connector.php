...
namespace NigiriElatedNamespace\Modules\Header\Lib;

/**
 * Class NigiriElatedClassHeaderConnector
 *
...
class NigiriElatedClassHeaderConnector {
	...
	public function connect( $config = array() ) {
		do_action( 'nigiri_elated_action_pre_header_connect' );
		...
		if ( ! empty( $config['affect_content'] ) ) {
			add_filter( 'nigiri_elated_filter_content_elem_style_attr', array( $this, 'contentMarginFilter' ) );
		}
		
		if ( ! empty( $config['affect_title'] ) ) {
			add_filter( 'nigiri_elated_filter_title_content_padding', array( $this, 'titlePaddingFilter' ) );
		}
		
		do_action( 'nigiri_elated_action_after_header_connect' );
	}
	 ...
	 *
	 * Hooks to nigiri_elated_filter_title_content_padding filter
	 *
	 * @return int
	 */
