...
class HeaderStandardExtended extends HeaderType {
	...
	public function __construct() {
		...
		if ( ! is_admin() ) {
			$this->logoAreaHeight     = nigiri_elated_set_default_logo_height_for_header_types();
	...
	public function loadTemplate( $parameters = array() ) {
		...
		$parameters['logo_area_in_grid'] = nigiri_elated_get_meta_field_intersect( 'logo_area_in_grid', $id ) == 'yes' ? true : false;
		$parameters['menu_area_in_grid'] = nigiri_elated_get_meta_field_intersect( 'menu_area_in_grid', $id ) == 'yes' ? true : false;
		
		$parameters = apply_filters( 'nigiri_elated_filter_header_standard_extended_parameters', $parameters );
	...
	public function calculateHeightOfTransparency() {
		...
		$logo_background_color             = nigiri_elated_get_meta_field_intersect( 'logo_area_background_color', $id );
		$logo_background_transparency      = nigiri_elated_get_meta_field_intersect( 'logo_area_background_transparency', $id );
		$logo_grid_background_color        = nigiri_elated_options()->getOptionValue( 'logo_area_grid_background_color' );
		$logo_grid_background_transparency = nigiri_elated_options()->getOptionValue( 'logo_area_grid_background_transparency' );
	...
	public function calculateHeightOfCompleteTransparency() {
		...
		$logo_background_color             = nigiri_elated_options()->getOptionValue( 'logo_area_background_color' );
		$logo_background_transparency      = nigiri_elated_options()->getOptionValue( 'logo_area_background_transparency' );
		$logo_grid_background_color        = nigiri_elated_options()->getOptionValue( 'logo_area_grid_background_color' );
		$logo_grid_background_transparency = nigiri_elated_options()->getOptionValue( 'logo_area_grid_background_transparency' );
		...
		if ( $logoAreaTransparent || $menuAreaTransparent ) {
			if ( $logoAreaTransparent ) {
				...
				if ( nigiri_elated_is_top_bar_enabled() && nigiri_elated_is_top_bar_completely_transparent() ) {
					$transparencyHeight += nigiri_elated_get_top_bar_height();
				}
			}
