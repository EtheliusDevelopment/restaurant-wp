...
/*
   Class: NigiriElatedClassDashboardForm
   A class that initializes Elated Dashboard Container
*/
class NigiriElatedClassDashboardForm implements iNigiriElatedInterfaceLayoutNode, iNigiriElatedInterfaceRender {
	public $children;
	public $name;
	public $form_id;
	public $form_method;
...
/*
   Class: NigiriElatedClassDashboardGroup
   A class that initializes NigiriElatedClass Group Field
*/
class NigiriElatedClassDashboardGroup implements iNigiriElatedInterfaceLayoutNode, iNigiriElatedInterfaceRender{
	public $children;
	public $name;
	public $title;
	public $description;
...
/*
   Class: NigiriElatedClassDashboardTitle
   A class that initializes Dashboard Title
*/
class NigiriElatedClassDashboardTitle implements iNigiriElatedInterfaceRender {
	private $name;
	private $title;
...
/*
   Class: NigiriElatedClassDashboardField
   A class that initializes NigiriElatedClass Front Field
*/

class NigiriElatedClassDashboardField implements iNigiriElatedInterfaceRender {
	private $type;
	private $name;
	private $label;
	private $description;
...
abstract class NigiriElatedClassDashboardFieldType {
	abstract public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = "" );
}

class NigiriElatedClassDashboardFieldText extends NigiriElatedClassDashboardFieldType {
...
class NigiriElatedClassDashboardFieldTextArea extends NigiriElatedClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$col_width = 12;
...
class NigiriElatedClassDashboardFieldImage extends NigiriElatedClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
        $class = '';
			...
			<div class="eltdf-dashboard-gallery-holder">
				<ul class="eltdf-dashboard-gallery-images-holder">
					<?php if ( isset($value_html) ) {
                        echo nigiri_elated_get_module_part( $value_html ) ;
					} ?>
				</ul>
				<div class="eltdf-dashboard-gallery-uploader">
					<?php 
					if ( nigiri_membership_theme_installed() ) {
						echo nigiri_elated_get_button_html( array(
							'text'      => esc_html__( 'Upload', 'nigiri' ),
							'custom_class' => 'eltdf-dashboard-gallery-upload'
						) );
					} else {
						echo '<a itemprop="url" href="#" class="eltdf-btn eltdf-btn-medium eltdf-btn-solid eltdf-dashboard-gallery-upload"><span class="eltdf-btn-text">' . esc_html__( 'Upload', 'nigiri' ) . '</span></a>';
					} ?>
					...
					<?php if ($value !== '' && $value !== false) { ?>
						<button class="eltdf-btn eltdf-btn-solid eltdf-dashboard-remove-image"><?php esc_html_e('Remove Media','nigiri'); ?></button>
					<?php } ?>
				</div>
			</div>
		</div>
...
class NigiriElatedClassDashboardFieldGallery extends NigiriElatedClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
        $class = '';
...
class NigiriElatedClassDashboardFieldSelect extends NigiriElatedClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
        $class = '';
...
class NigiriElatedClassDashboardFieldDate extends NigiriElatedClassFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
        if (!empty($repeat) && array_key_exists('name', $repeat) && array_key_exists('index', $repeat)) {
			$id		= $name . '-' . $repeat['index'];
			$name	= $repeat['name'] . '['.$repeat['index'].']['. $name .']';
...
class NigiriElatedClassDashboardFieldIcon extends NigiriElatedClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$options           = nigiri_elated_icon_collections()->getIconCollectionsEmpty();
		$icons_collections = nigiri_elated_icon_collections()->getIconCollectionsKeys();
				...
				<div class="eltdf-dashboard-item">
					...
					<div class="eltdf-dashboard-icon-holder">
						...
						<?php foreach ( $icons_collections as $icons_collection ) {
							$icons_param = nigiri_elated_icon_collections()->getIconCollectionParamNameByKey( $icons_collection );
	                        $style       = 'display: none';
							...
							<div class="eltdf-icon-collection-holder" style="<?php echo esc_attr( $style ); ?>" data-icon-collection="<?php echo esc_attr( $icons_collection ); ?>">
                                <select name="<?php echo esc_attr( $name . '[' . $icons_param . ']'); ?>" id="<?php echo esc_attr( $name . '[' . $icons_param . ']'); ?>">
									<?php
									$icons = nigiri_elated_icon_collections()->getIconCollection( $icons_collection );
...
class NigiriElatedClassDashboardFieldColor extends NigiriElatedClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
        if (!empty($repeat) && array_key_exists('name', $repeat) && array_key_exists('index', $repeat)) {
			$id		= $name . '-' . $repeat['index'];
			$name	= $repeat['name'] . '['.$repeat['index'].']['. $name .']';
...
class NigiriElatedClassDashboardFieldCheckBoxGroup extends NigiriElatedClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$col_width = 12;
...
class NigiriElatedClassDashboardFieldAddress extends NigiriElatedClassFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$col_width = 12;
		if ( isset( $args["col_width"] ) ) {
			$col_width = $args["col_width"];
			...
			<div class="eltdf-dashboard-field-row">
				<div class="eltdf-dashboard-item col-lg-<?php echo esc_attr($col_width); ?>">
					<div class="eltdf-dashboard-input-holder">
						...
						<a id="reset" href="#" style="display:none;"><?php esc_html_e( 'Reset Marker', 'nigiri' ); ?></a>
					</div>
				</div>
			</div>
		</div>
...
class NigiriElatedClassDashboardRepeater implements iNigiriElatedInterfaceRender {
	private $label;
	private $description;
	private $name;
	private $fields;
	...
	function __construct( $fields, $name, $label = '', $description = '', $button_text = '', $table_layout = false, $value = array() ) {
		...
		$this->name        = $name;
		$this->num_of_rows = 1;
		$this->button_text = ! empty( $button_text ) ? $button_text : esc_html__( 'Add New Item', 'nigiri' );
			...
			<?php if ($this->table_layout) { ?>
				<div class="eltdf-dashboard-repeater-table-heading">
					<div class="eltdf-dashboard-repeater-fields-holder">
						<div class="eltdf-dashboard-repeater-table-cell eltdf-dashboard-repeater-sort"><?php esc_html_e( 'Order', 'nigiri' ) ?></div>
						...
						<div class="eltdf-dashboard-repeater-table-cell eltdf-dashboard-repeater-remove"><?php esc_html_e( 'Remove', 'nigiri' ) ?></div>
					</div>
				</div>
			<?php } ?>
...
class NigiriElatedClassDashboardFieldFactory {
	public function render( $field_type, $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array()) {
		switch ( strtolower( $field_type ) ) {
			case 'text':
				$field = new NigiriElatedClassDashboardFieldText();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'textarea':
				$field = new NigiriElatedClassDashboardFieldTextArea();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'date':
				$field = new NigiriElatedClassDashboardFieldDate();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'image':
				$field = new NigiriElatedClassDashboardFieldImage();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'gallery':
				$field = new NigiriElatedClassDashboardFieldGallery();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'select':
				$field = new NigiriElatedClassDashboardFieldSelect();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'icon':
				$field = new NigiriElatedClassDashboardFieldIcon();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'color':
				$field = new NigiriElatedClassDashboardFieldColor();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'checkboxgroup':
				$field = new NigiriElatedClassDashboardFieldCheckBoxGroup();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'address':
				$field = new NigiriElatedClassDashboardFieldAddress();
