...
if ( ! function_exists( 'nigiri_elated_get_icon_pack_collections' ) ) {
	function nigiri_elated_get_icon_pack_collections() {
		$options = apply_filters( 'nigiri_elated_filter_add_icon_pack_into_collection', $options = array() );
		
		return $options;
	}
	
	add_action( 'after_setup_theme', 'nigiri_elated_get_icon_pack_collections' );
}

if ( ! function_exists( 'nigiri_elated_get_icon_pack_options' ) ) {
	function nigiri_elated_get_icon_pack_options() {
		$options = apply_filters( 'nigiri_elated_filter_add_icon_pack_into_options', $options = array() );
		
		return $options;
	}
	
	add_action( 'after_setup_theme', 'nigiri_elated_get_icon_pack_options' );
}

if ( ! function_exists( 'nigiri_elated_get_icon_pack_social_options' ) ) {
	function nigiri_elated_get_icon_pack_social_options() {
		$options = apply_filters( 'nigiri_elated_filter_add_icon_pack_into_social_options', $options = array() );
		
		return $options;
	}
	
	add_action( 'after_setup_theme', 'nigiri_elated_get_icon_pack_social_options' );
}

/*
 * Class: NigiriElatedClassIconCollections
 */
class NigiriElatedClassIconCollections {
	...
	private function __construct() {
		...
		$this->iconPackCollections   = nigiri_elated_get_icon_pack_collections( true );
		$this->iconPackOptions       = nigiri_elated_get_icon_pack_options();
		$this->iconPackSocialOptions = nigiri_elated_get_icon_pack_social_options();
	...
	public function getIconsMetaBoxOrOption( $attributes ) {
		...
		foreach ( $icon_collections as $collection_key => $collection_object ) {
			...
			$icon_pack_container = nigiri_elated_add_admin_container(
				array(
					'parent'     => $parent,
	...
	public function getVCParamsArray( $iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false ) {
		...
		$iconPackParams = array(
			'type'        => 'dropdown',
			'heading'     => esc_html__( 'Icon Pack', 'nigiri' ),
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'        => 'dropdown',
					'heading'     => esc_html__( 'Icon', 'nigiri' ),
	...
	public function getIconWidgetParamsArray() {
		$iconPackParams[] = array(
			'type'    => 'dropdown',
			'name'    => 'icon_pack',
			'title'   => esc_html__( 'Icon Pack', 'nigiri' ),
			'options' => $this->getIconPackOptions()
		);
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'    => 'dropdown',
					'title'   => $collection->title . esc_html__( ' Icon', 'nigiri' ),
	...
	public function getSocialIconWidgetMultipleParamsArray( $count ) {
		...
		for ( $n = 1; $n <= $count; $n ++ ) {
			if ( is_array( $iconCollectionsVC ) && count( $iconCollectionsVC ) ) {
				foreach ( $iconCollectionsVC as $key => $collection ) {
					$iconOps[] = array(
						...
						'title'   => sprintf( esc_html__( 'Icon %s %s Icon', 'nigiri' ), $n, $collection->title ),
			...
			$iconOps[] = array(
				'type'  => 'textfield',
				'name'  => 'text_' . $n,
				'title' => sprintf( esc_html__( 'Text %s', 'nigiri' ), $n )
			);

			$iconOps[] = array(
				'type'  => 'textfield',
				'name'  => 'link_' . $n,
				'title' => sprintf( esc_html__( 'Link %s', 'nigiri' ), $n )
			);
			
			$iconOps[] = array(
				'type'    => 'dropdown',
				'name'    => 'target_' . $n,
				'title'   => sprintf( esc_html__( 'Link Target %s', 'nigiri' ), $n ),
	...
	public function getIconSizesArray() {
		return array(
			esc_html__( 'Tiny', 'nigiri' )       => 'fa-lg',
			esc_html__( 'Small', 'nigiri' )      => 'fa-2x',
			esc_html__( 'Medium', 'nigiri' )     => 'fa-3x',
			esc_html__( 'Large', 'nigiri' )      => 'fa-4x',
			esc_html__( 'Very Large', 'nigiri' ) => 'fa-5x'
		);
	}
	...
	public function getIconCollectionIcons( iNigiriElatedInterfaceIconCollection $collection ) {
	...
	public function getIconCollectionsEmpty( $no_empty_key = "" ) {
		$array                  = array();
		$array[ $no_empty_key ] = esc_html__( 'No Icon', 'nigiri' );
	...
	public function getIconCollectionsVCEmpty() {
		$vc_array                                           = array();
		$vc_array[ esc_html__( 'No Icon', 'nigiri' ) ] = '';
