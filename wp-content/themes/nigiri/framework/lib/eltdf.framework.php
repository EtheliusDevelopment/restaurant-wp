...
/*
   Class: NigiriElatedClassFramework
...
class NigiriElatedClassFramework {
	...
	private function __construct() {
		$this->eltdOptions         = NigiriElatedClassOptions::get_instance();
		$this->eltdMetaBoxes       = NigiriElatedClassMetaBoxes::get_instance();
		$this->eltdTaxonomyOptions = NigiriElatedClassTaxonomyOptions::get_instance();
		$this->eltdUserOptions     = NigiriElatedClassUserOptions::get_instance();
		$this->eltdDashboardOptions = NigiriElatedClassDashboardOptions::get_instance();
	}
	...
	public function setSkin( NigiriElatedClassSkinAbstract $skinObject ) {
		$this->skin = $skinObject;
	}
}

/**
 * Class NigiriElatedClassSkinManager
 *
...
class NigiriElatedClassSkinManager {
	...
	/**
	 * @see NigiriElatedClassSkinManager::setSkin()
	 */
	...
	private function setSkin( $skinName = ELATED_PROFILE_SLUG ) {
		if ( $skinName !== '' ) {
			if ( file_exists( get_template_directory() . '/framework/admin/skins/' . $skinName . '/skin.php' ) ) {
				...
				$skinName = ucfirst( $skinName ) . esc_html__( 'Skin', 'nigiri' );
...
/**
 * Class NigiriElatedClassSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class NigiriElatedClassSkinAbstract {
	/**
	 * @var string
	 */
	protected $skinName;
	...
	public function loadTemplatePart( $template, $params = array() ) {
		...
		if ( $template !== '' ) {
			include( nigiri_elated_find_template_path( 'framework/admin/skins/' . $this->skinName . '/templates/' . $template . '.php' ) );
		}
	}
	...
	public function setShortcodeJSParams() { ?>
		<script>
			window.eltdSCIcon = '<?php echo nigiri_elated_get_skin_uri() . '/assets/img/admin-logo-icon.png'; ?>';
			window.eltdSCLabel = '<?php echo esc_html( ucfirst( $this->skinName ) ); ?> <?php esc_html_e( 'Shortcodes', 'nigiri' ) ?>';
		</script>
	<?php }
...
/*
   Class: NigiriElatedClassOptions
...
class NigiriElatedClassOptions {
	...
	public function getOptionValue( $key ) {
		...
		if ( array_key_exists( $key, $nigiri_elated_global_options ) ) {
			return $nigiri_elated_global_options[ $key ];
...
/*
   Class: NigiriElatedClassAdminPage
   A class that initializes Elated Admin Page
*/
class NigiriElatedClassAdminPage implements iNigiriElatedInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $slug;
	public $title;
	...
	function __construct( $slug = "", $title_label = "", $icon = "" ) {
		$this->layout  = array();
		$this->factory = new NigiriElatedClassFieldFactory();
		$this->slug    = $slug;
	...
	public function renderChild( iNigiriElatedInterfaceRender $child ) {
		$child->render( $this->factory );
	}
}

/*
   Class: NigiriElatedClassMetaBoxes
...
/*
   Class: NigiriElatedClassMetaBox
   A class that initializes Elated Meta Box
*/
class NigiriElatedClassMetaBox implements iNigiriElatedInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	public $title;
	...
	public function setTitle( $label ) {
		...
		return $nigiri_elated_global_Framework->getSkin()->getSkinLabel() . ' ' . $label;
	}
}

/*
   Class: NigiriElatedClassTaxonomyOptions
   A class that initializes NigiriElatedClass Taxonomy Options
*/
class NigiriElatedClassTaxonomyOptions {
	private static $instance;
...
/*
   Class: NigiriElatedClassTaxonomyOption
   A class that initializes NigiriElatedClass Taxonomy Option
*/
class NigiriElatedClassTaxonomyOption implements iNigiriElatedInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	...
	function __construct( $scope = "" ) {
		$this->layout  = array();
		$this->factory = new NigiriElatedClassTaxonomyFieldFactory();
		$this->scope   = $scope;
	}
...
/*
   Class: NigiriElatedClassUserOptions
   A class that initializes NigiriElatedClass User Options
*/
class NigiriElatedClassUserOptions {
	private static $instance;
	public $userOptions;
...
/*
   Class: NigiriElatedClassUserOption
   A class that initializes NigiriElatedClass User Option
*/
class NigiriElatedClassUserOption implements iNigiriElatedInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	...
	function __construct( $scope = "" ) {
		$this->layout  = array();
		$this->factory = new NigiriElatedClassUserFieldFactory();
		$this->scope   = $scope;
	}
...
/*
   Class: NigiriElatedClassDashboardOptions
   A class that initializes NigiriElatedClass Dashboard Options
*/
class NigiriElatedClassDashboardOptions {
	private static $instance;
...
/*
   Class: NigiriElatedClassDashboardOption
   A class that initializes NigiriElatedClass Dashboard Option
*/
class NigiriElatedClassDashboardOption implements iNigiriElatedInterfaceLayoutNode {
	public $layout;
	private $factory;
	
	function __construct() {
		$this->layout        = array();
		$this->factory       = new NigiriElatedClassDashboardFieldFactory();
	}
...
if ( ! function_exists( 'nigiri_elated_init_framework_variable' ) ) {
	function nigiri_elated_init_framework_variable() {
		global $nigiri_elated_global_Framework;
		
		$nigiri_elated_global_Framework = NigiriElatedClassFramework::get_instance();
		$eltdSkinManager                 = new NigiriElatedClassSkinManager();
		$nigiri_elated_global_Framework->setSkin( $eltdSkinManager->getSkin() );
	}
	
	add_action( 'nigiri_elated_action_before_options_map', 'nigiri_elated_init_framework_variable' );
}
?>