...
class ElatedSkin extends NigiriElatedClassSkinAbstract {
	
    /**
     * Skin constructor. Hooks to nigiri_elated_admin_scripts_init and nigiri_elated_enqueue_admin_styles
     */
	public function __construct() {
		$this->skinName = 'elated';
		
		//hook to
		add_action( 'nigiri_elated_action_admin_scripts_init', array( $this, 'registerStyles' ) );
		add_action( 'nigiri_elated_action_admin_scripts_init', array( $this, 'registerScripts' ) );
		
		add_action( 'nigiri_elated_action_enqueue_admin_styles', array( $this, 'enqueueStyles' ) );
		add_action( 'nigiri_elated_action_enqueue_admin_scripts', array( $this, 'enqueueScripts' ) );
		
		add_action( 'nigiri_elated_action_enqueue_meta_box_styles', array( $this, 'enqueueStyles' ) );
		add_action( 'nigiri_elated_action_enqueue_meta_box_scripts', array( $this, 'enqueueScripts' ) );
		
		add_filter( 'nigiri_elated_filter_options_map_position', array( $this, 'setOptionsMapPosition' ), 10, 2 );
	...
	public function registerScripts() {
		...
		foreach ( $this->scripts as $scriptHandle => $script ) {
			nigiri_elated_register_skin_script( $scriptHandle, $script['path'], $script['dependency'] );
		}
	}
    ...
    public function registerStyles() {
        ...
        foreach ($this->styles as $styleHandle => $stylePath) {
	        nigiri_elated_register_skin_style($styleHandle, $stylePath);
        }
    }
    ...
    public function renderOptions() {
        ...
        $tab    = nigiri_elated_get_admin_tab();
        $active_page = $nigiri_elated_global_Framework->eltdOptions->getAdminPageFromSlug($tab);
     ...
     *
     * @see NigiriElatedClassSkinAbstract::loadTemplatePart()
     */
	 ...
	 *
	 * @param NigiriElatedClassAdminPage $page current page to load
	 * @see NigiriElatedClassSkinAbstract::loadTemplatePart()
	 */
